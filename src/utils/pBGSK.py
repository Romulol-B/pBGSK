...
from .feature_selector_evaluator import FeatureSelectorEvaluator
...
class Individual:
    ...
    def __repr__(self):
        return (
            ...
            f"Binary feature vector:{self.features + 0}"
        )  # conversion to int.
...
class CrossValidatedFeatureSelectorEvaluator:
    ...
    def __init__(
        ...
        if self.X.ndim != 2:
            raise ValueError(
                "X must be a two-dimensional array with shape (n_samples, n_features)."
            )
...
def __getattr__(name):
    if name == "PBGSKFeatureSelector":
        from .pbgsk_feature_selector import PBGSKFeatureSelector

        return PBGSKFeatureSelector
